...
class analitica():
    ...
    def operaciones(self, sensor):
        ...
        if ("max-{}".format(sensor)=="max-humedad".format(sensor)) and str(df_filtrado.max(skipna = True))>"95":
            publish.single('2/alertahum', "Precaucion Humedad elevada", hostname='40.74.216.31', client_id='pub_alerta')
        ...
        if ("min-{}".format(sensor)=="min-humedad".format(sensor)) and str(df_filtrado.min(skipna = True))<"60":
            publish.single('2/alertahum', "Precaucion Humedad baja", hostname='40.74.216.31', client_id='pub_alerta')
            #self.publicar("alerta-humedad".format(sensor), "Precaucion Humedad Muy Baja")

        if ("max-{}".format(sensor)=="max-temperatura".format(sensor)) and str(df_filtrado.max(skipna = True))>"31":
            publish.single('2/alertatemp', "Precaucion temperatura elevada", hostname='40.74.216.31', client_id='pub_alerta')
            #self.publicar("alerta-temperatura".format(sensor), "Precaucion temperatura Elevada ")

        if ("min-{}".format(sensor)=="min-temperatura".format(sensor)) and str(df_filtrado.min(skipna = True))<"20":
            publish.single('2/alertatemp', "Precaucion temperatura baja", hostname='40.74.216.31', client_id='pub_alerta')
